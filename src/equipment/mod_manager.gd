...
func _init(owner: EquipmentBase):
...
func install_mod(mod_resource: ModResource, slot_index: int = -1) -> int:
	...
	# 检查兼容性
	if not _check_compatibility(mod_resource):
		print("警告: 模组与装备不兼容: ", mod_resource.mod_name)
		print("  装备标签数量: ", equipment_owner.equipment_tags.size())
		print("  模组兼容标签数量: ", mod_resource.compatible_tags.size())
		return -1
...
## [returns] 是否兼容
func _check_compatibility(mod_resource: ModResource) -> bool:
	if not equipment_owner:
		return false
	
	return mod_resource.is_compatible_with_equipment(equipment_owner.equipment_tags)
